...
##############################Legend############################################
###Read Data########################
Data<-rio::import(paste("Data/Hasard_Exposure.csv",sep=""))
Data$Risk=(Data$HE * 5.83)^(1/2)
Data$Risk=(Data$Risk-min(Data$Risk))/(max(Data$Risk)-min(Data$Risk))
Raster_file<-rasterFromXYZ(Data[c("x","y","Risk")])
...
rio::export(Data,"Data/Risk.csv")
#rio::export(Data,"Data/95thperc_Data_Nigeria_CHIRPS.csv")
mybreaks <- c(0,0.2,0.4,0.5,0.6,0.8,1,Inf)
...
breaklabel <- function(x){
  labels<-as.character(c(0,0.2,0.4,0.5,0.6,0.8,1))
  #labels<- as.character(seq(1,5))
  labels[1:x]
}
################################################################################

Title<-paste("Risk Over Kaduna","\nRef: 1983-2021","\nData Source: ",Data_Source,sep="")

#Im<-grid::rasterGrob(png::readPNG("Logos/Acmad_logo_1.png"), interpolate = TRUE)

l<-ggplot()+geom_contour_filled(data=Data, aes(x,y,z =Risk),breaks= mybreaks, show.legend = TRUE) +
...
jpeg(filename = paste("Products/Graphs/","Risk_Over_Kaduna_","_",Data_Source,".jpeg",sep=""),
     width = 14,
     height =14,
     units = "in",
     res=300)
