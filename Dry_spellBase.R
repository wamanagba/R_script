...
for (k in 1981:2010) {
  print(k);print(season)
    ...
    Data_F=merge(Data_Dry_Speel,Mask,by=c("X","Y"))
    DATA_F=filter(Data_F,mask>=1)
    dir.create(paste("Data/CPC-UNIFIED/DataBase/Number_DrySpell10/",season,sep = ""),recursive = T,showWarnings = F)
    rio::export(DATA_F,paste("Data/CPC-UNIFIED/DataBase/Number_DrySpell10/",season,"/",k,".csv",sep = ""))
...
DATA_F=dd
DATA_F=filter(DATA_F,Spell>=0)


Raster_file<-rasterFromXYZ(DATA_F[c("X","Y","Spell")])
...
breaklabel <- function(x){
  labels<- as.character(c(0,1,2,3,4,5,6))
  labels[1:x]
}
################################################################################

#Title<-paste("Number of dry spell overs 10 days", "\nRef: 1981-2010","\nData Source: ",Data_Source,"\n Season:",season,sep="")
Title<-toupper(paste("Dry Spells over Africa \nData Source: ",Data_Source,sep=""))

#Im<-grid::rasterGrob(png::readPNG("Logos/Acmad_logo_1.png"), interpolate = TRUE)

l<-ggplot()+geom_contour_filled(data=Data_F, aes(x,y,z =Spell),breaks= mybreaks, show.legend = TRUE) +
...
jpeg(filename = paste("Data/CPC-UNIFIED/DataBase/Number_DrySpell10/",season,"/",k,".jpeg",sep=""),
     width = 14,
     height =14,
     units = "in",
     res=300)
