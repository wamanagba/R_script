...
for(cpt in 1:12){
  ...
  for (k in 1981:2010) {
    ...
    if(k==1981){
      ...
      Data_Dry_Speel<-Data%>%
        group_by(X,Y)%>%
        summarise(Mean=CDD_function(rain,2.5,Nb=10))
      
      Data_F<-merge(Sum,Data_Dry_Speel,by=c("X","Y")) 
      Data_F$clim= ifelse(Data_F$clim>100,1,NA)
      dir.create(paste("Data/Dry_Speel_Data/By_year/10Days/",season,"/",sep = ""),recursive = T, showWarnings = F)
      rio::export(Data_F,paste("Data/Dry_Speel_Data/By_year/10Days/",season,"/",k,".csv",sep = ""))
      data=filter(Data_F,clim==1)
      ...
      Data_F<-merge(Sum,Data_Dry_Speel,by=c("X","Y")) 
      Data_F$clim= ifelse(Data_F$clim>100,1,NA)
      dir.create(paste("Data/Dry_Speel_Data/By_year/10Days/",sep = ""),recursive = T, showWarnings = F)
      rio::export(Data_F,paste("Data/Dry_Speel_Data/By_year/10Days/Dry_Spell_",k,"_",season,".csv",sep = ""))
      data=filter(Data_F,clim==1)
    }
    ...
    dir.create(paste("Products/Dry_speel/Afrique/10Days/",season,"/",sep=""),recursive = T,showWarnings = F)
    
    jpeg(filename = paste("Products/Dry_speel/Afrique/10Days/",season,"/",season,"_",k,".jpeg",sep=""),
         width = 14,
         height =14,
         units = "in",
         res=300)
